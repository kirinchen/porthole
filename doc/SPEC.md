...
- **選字插入 / 編輯連結**(CM6 編輯 + 表格 cell):選取文字後**右鍵**點選區 → 於滑鼠處浮出「🔗 連結」鈕(取代原生選單;純 DOM 共用 `lib/linkTooltip`,不引 Antd 進 CM6 樹)。點鈕派 `porthole:edit-link` 事件 → 由 `MarkdownEditor` 的 antd Modal 接手(連結文字 + 網址;網址必填才可套用),套用以 `[文字](網址)` 寫回。**來源解耦**:`detail.apply(md)` callback —— CM6 走 `view.dispatch` 取代範圍(選區落在既有 `Link` 節點內則整段替換 + `linkNodeAt` 預填 href);**表格 cell**(`TableBlock` widget 為 `MarkdownEditor` 子樹,共用同一 Modal)走改寫 cell 子字串(選取本身已是 `[t](u)` 則 `parseInlineLink` 預填)。右鍵不清選取,故不需保焦;detail 於右鍵當下就建好(選區已知),之後失焦不影響套用。無選取的右鍵 → 交給 flowContextMenu(空白行圖表選單)/ 原生選單。
