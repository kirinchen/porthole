...
- **新 session**:`+ 新 session` → 在 repo cwd 開一個**裸 shell 背景 tmux** 並 attach(不自動跑 claude,要的話使用者自行下)。系統開的這類 tmux 在清單以 `$` 前綴標示;若在內跑 claude 並互動,claude 自落 `.jsonl` 後即另列入可恢復清單。
...
- **生命週期**:
  ...
  - 建立(新):`tmux new-session -d -s <name> -c <repoRoot>`(`POST /api/:repo/sessions/new`,裸 shell,不自動跑 claude);清單以 `$` 前綴顯示,若使用者在內跑 claude 並互動則另落 `.jsonl` 列入可恢復清單。
