...
- 點**資料夾** → 中央顯示該夾內容(可點:檔案開檔、子夾鑽入)+ 其 `README.md`(若有,case-insensitive)渲染於下。
  ...
  - **編輯 markdown 時貼上圖片**:CM6 編輯器內貼上剪貼簿圖片 → 存到**目前檔所在目錄**(`pasted-<YYYYMMDD-HHMMSS>.<ext>`,base64,path-guard),並於游標插入 `![](檔名)`(同目錄故相對路徑=檔名);非圖片內容放行(正常貼文字)。**預覽渲染相對圖片**:`Markdown` 的 `img` 以「目前開啟檔」為基準解析相對路徑成 `GET /api/:repo/raw`(`MdImg`,同 `MdLink`),外部 / `data:` 原樣 —— 否則相對於瀏覽器 URL 會 404。
