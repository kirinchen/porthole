...
- **編輯**:預覽區「編輯」鈕 → markdown 走 CM6 Obsidian 式 live-preview、其他純 textarea → 儲存(`PUT /api/:repo/file`)。「儲存」鈕存檔並回預覽;**Ctrl/Cmd+S** 存檔但**不離開編輯**(派 `porthole:save-file`,讀 draftRef)。進編輯**延續 preview 捲動位置**:取 preview 頂端當前 heading(第 N 個)→ 對應原始碼第 N 個標題行(`headingLineNumbers`,同 `parseHeadings` 跳 fenced code 規則),CM6 mount 後捲到該行並置游標行首。
