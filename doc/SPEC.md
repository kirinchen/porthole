...
- **選字插入 / 編輯連結**(CM6 編輯):選取非空且同一行的文字 → 選區上方浮出「🔗 連結」鈕(純 DOM，不引 Antd 進 CM6 樹；`mousedown` `preventDefault` 保住選區)。點鈕派 `porthole:edit-link` 事件 → 開 antd Modal(連結文字 + 網址；網址必填才可套用),套用以 `[文字](網址)` 取代原範圍。選區落在既有 `Link` 節點內時整段替換並**預填現有 href**(`linkNodeAt` 走 syntaxTree)。連結文字不可跨行,故僅同行選取顯示;失焦 / 空選 / 捲動即收起浮動鈕。
